...
import { login } from "../Store/features/counter/userSlice";
...
function Login() {
  ...
  const [passwordForReg, setPasswordForReg] = useState("");
  ...
  const register = () => {
    ...
    //return alert('Please enter your name'+emailForReg + passwordForReg);

    createUserWithEmailAndPassword(auth, emailForReg, passwordForReg)
      .then((userCredential) => {
        updateProfile(userCredential.user, {
          ...
          .then(() => {
            dispatch(
              login({
                email: userCredential.user.emailForReg,
          ...
          .catch((error) => {
            return alert(
              "Error:" + error.message + " || " + emailForReg + passwordForReg
            );
          });
        ...
        setPasswordForReg("");
        setPhotoUrl("");
        return alert("Your Account has been create successfuly!");
      })
      .catch((error) => {
  ...
  return (
    <div className="login">
      ...
      <div className="tagline">
        <h1 className="tagline_text">
          Stay updated on your professional world
        </h1>
      </div>
      <div className="footer_text">
        <h5>
          Build by <span>Bukhtyar Haider</span>
        </h5>
      </div>
      <div className="container">
        <div className="login_body">
          <form className="login_box">
            ...
            <button type="submit" onClick={loginto} className="login_signinbtn">
              Sign in
            </button>
          </form>
          <h5>
        ...
        <div className="register_body">
          <div className="register_box">
            ...
            <input
              type="text"
              value={emailForReg}
              onChange={(e) => setEmailForReg(e.target.value)}
              placeholder="Email or Phone"
            />
            <input
              type="password"
              value={passwordForReg}
              onChange={(e) => setPasswordForReg(e.target.value)}
              placeholder="Password"
            />
            <p>
            ...
            <button onClick={register} className="register_signinbtn">
              Agree & Join
            </button>
          </div>
          <h5>
